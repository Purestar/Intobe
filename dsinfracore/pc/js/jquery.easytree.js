    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        function nodeClick(event) {
            ...
            if (_settings.stateChanged) { // fire stateChanged event
                ...
                _settings.stateChanged(nodes, node);
            }
            if (_settings.isCheckBox) { // fire Checkbox Click event
            	var j = getMinifiedJson(nodes);
        ...
        function eventCheckbox(node){

    		var obj = $('#' + node.id).parents('li').first().find("input[type=checkbox]");
    		var checked = $(obj).prop("checked");
    		var limit = obj.attr("tree-level") == "" ? "0" : obj.attr("tree-level");
    		
    		var parentKey = node.pKey;
    		
    		// 상위 Node Checkbox
    		for(var i = limit; i >= 1; i--){
    			var parentObj = $("#tree_menu #tree_" + parentKey);
    			var pChk = true;
    			var iChk = 0;
    			var iTmp = 0;
    			
    			$("#tree_menu .tree-level" + i).each(function(){
    				if(!checked){
    					if($(this).attr("pKey") == parentKey) {
    						iChk++;
    			...
    			$(parentObj).prop("checked", pChk);
    			parentKey = $(parentObj).attr("pKey");
    		}
        }
        ...
        function buildTree(nodes) {
            ...
            $(".easytree-node").on("click", nodes, nodeClick);
            $(".easytree-expander").on("click", nodes, toggleNodeEvt);
            $(".easytree-icon").on("dblclick", nodes, toggleNodeEvt);
            $(".easytree-title").on("dblclick", nodes, toggleNodeEvt);
...
//                _settings.stateChanged(nodes, j);
        ...
        function getTitleHtml(node) {
            ...
            if(_settings.isCheckBox){
            	var treeKey = "";
            	if(node.key){
            		treeKey = $.trim(node.key);
            	}
            	html += "<input type='checkbox' id='tree_" + treeKey + "' name='tree_" + treeKey + "' class='treeClass tree-level" + node.level +"' value='" + treeKey +"' tree-level='" + node.level +"' pkey='" + node.pKey+"'  />&nbsp;";
            }
            if (node.code)
            	html += node.text + "<span style='color:red;padding-left: 5px;font-size: 80%;vertical-align: middle;'>(" + node.code + ")</span>";
            else
            	html += node.text
