...
$(function () {
	...
	$("[data-scr-fix]").scrFix();
	$(".tab-wrap").tabUI(); // 탭메뉴
	$("[class*='tab-menu']").tabActive(); // 탭 활성화
	...
	$("[class*='stk-']").scrStkFix(); // Sticky 감지
	$(window).hrzOff(); // 가로모드 방지
});
...
$.fn.tabUI = function (options) {
	...
	if (!hasAttr) {
		...
		$obj.on("click", "[class*='tab-menu'] button, [class*='tab-menu'] a", function () {
			...
			gap = $(this).closest("[class*='tab-menu']").offset().top + $(this).closest("[class*='tab-menu']").height();

			$menu.find("li").removeClass("active").eq(idx).addClass("active");
			$cont.find("> *").removeClass("active").eq(idx).addClass("active");
			
			// Sticky + 앵커 이동일 경우
			if ($(this).hasAttr($(this), "href") && $($(this).attr('href')).length) {
				if ($(this).closest("[class*='stk-']").hasClass("fixed")) {
					setTimeout(function () {
						$("body").scrollTop($("body").scrollTop() - gap);
					}, 5);
				} else {
					gap = parseInt($(this).closest("[class*='stk-']").css("top")) + $(this).closest("[class*='tab-menu']").height();
					setTimeout(function () {
						$("body").scrollTop($("body").scrollTop() - gap);
					}, 5);
				};
...
// Tab Active (Horizontal Scroll)
$.fn.tabActive = function () {
	var $obj = $(this),
		$actTg = $obj.find("> .active"),
		
		tgLeft = 0
	;
	
	if ($obj.closest(".scr-hor").length) {
		tgLeft = $actTg.offset().left
		
		$obj.parent().scrollLeft(tgLeft);
	};
};
...
$.fn.hrzOff = function () {
	...
	$obj.on("orientationchange", function () {
